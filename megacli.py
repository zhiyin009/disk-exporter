...
def collect(registry: prometheus_client.CollectorRegistry):
    info = (subprocess.check_output(["/usr/bin/MegaCli64", "-AdpAllInfo", "-aAll", "-nolog"]).decode("utf-8").splitlines())
    pdlist = (subprocess.check_output(["/usr/bin/MegaCli64", "-PdList", "-aAll", "-nolog"]).decode("utf-8").splitlines())

    for timeseries in GAUGES.values():
        timeseries.clear()

    textfile_dir = Path("./out/")
    textfile_dir.mkdir(exist_ok=True)
    ...
    pat_info = [
        ...
        {
            "regex":
            re.compile("^Product\s+Name\s+:"),
            "action": [
                'out["megacli_controller"]["metrics"].append({ "labels": { "adapter": adapter, "product_name": line.split(":")[1].strip(), "package_build":"", "firmware_version":"", "bios_version":"", "serial_number":""}, "val": 1})'
            ],
        },
        {
            "regex":
            re.compile("^FW\s+Package\s+Build\s*:"),
            "action": [
                'out["megacli_controller"]["metrics"].append({ "labels": { "adapter": adapter, "package_build": line.split(":")[1].strip(), "product_name":"", "firmware_version":"", "bios_version":"", "serial_number":""}, "val": 1 })'
            ],
        },
        {
            "regex":
            re.compile("^FW\s+Version\s+:"),
            "action": [
                'out["megacli_controller"]["metrics"].append({ "labels": { "adapter": adapter, "firmware_version": line.split(":")[1].strip(), "product_name":"", "package_build":"", "bios_version":"", "serial_number":""}, "val": 1 })'
            ],
        },
        {
            "regex":
            re.compile("^BIOS\s+Version\s+:"),
            "action": [
                'out["megacli_controller"]["metrics"].append({ "labels": { "adapter": adapter, "bios_version": line.split(":")[1].strip(), "product_name":"", "package_build":"", "firmware_version":"", "serial_number":""}, "val": 1 })'
            ],
        },
        {
            "regex":
            re.compile("^Inquiry\s+Data\s+:"),
            "action": [
                'out["megacli_controller"]["metrics"].append({ "labels": { "adapter": adapter, "bios_version": "", "product_name":"", "package_build":"", "firmware_version":"", "serial_number": line.split(":")[1].strip()}, "val": 1 })'
            ],
        },
        {
            "regex": re.compile("^Memory\s+Size\s+:"),
            "action": ['out["megacli_memory_size_bytes"]["metrics"].append({ "labels": { "adapter": adapter, "type": "Total memory" }, "val": tobytes(line.split(":")[1].strip()) })'],
        },
        {
        ...
        },
        {
            ...
            "action": ['out["megacli_memory_size_bytes"]["metrics"].append({ "labels": { "adapter": adapter, "type": "Write cache" }, "val": tobytes(line.split(":")[1].strip()) })'],
        },
        {
            "regex": re.compile("^Virtual\s+Drives\s+:"),
            "action": ['out["megacli_drives"]["metrics"].append({ "labels": { "adapter": adapter, "type": "virtual", "state": "Total" }, "val": line.split(":")[1].strip() })'],
        },
        {
            "regex": re.compile("^\s+Degraded\s+:"),
            "action": ['out["megacli_drives"]["metrics"].append({ "labels": { "adapter": adapter, "type": "virtual", "state": "Degraded" }, "val": line.split(":")[1].strip() })'],
        },
        {
            "regex": re.compile("^\s+Offline\s+:"),
            "action": ['out["megacli_drives"]["metrics"].append({ "labels": { "adapter": adapter, "type": "virtual", "state": "Offline" }, "val": line.split(":")[1].strip() })'],
        },
        {
            "regex": re.compile("^Physical\s+Devices\s+:"),
            "action": ['out["megacli_drives"]["metrics"].append({ "labels": { "adapter": adapter, "type": "physical", "state": "PhysicalTotal" }, "val": line.split(":")[1].strip() })'],
        },
        {
            "regex": re.compile("^\s+Disks\s+:"),
            "action": ['out["megacli_drives"]["metrics"].append({ "labels": { "adapter": adapter, "type": "physical", "state": "DisksTotal"  }, "val": line.split(":")[1].strip() })'],
        },
        {
            "regex": re.compile("^\s+Critical\s+Disks\s+:"),
            "action": ['out["megacli_drives"]["metrics"].append({ "labels": { "adapter": adapter, "type": "physical", "state": "Critical"  }, "val": line.split(":")[1].strip() })'],
        },
        {
            "regex": re.compile("^\s+Failed\s+Disks\s+:"),
            "action": ['out["megacli_drives"]["metrics"].append({ "labels": { "adapter": adapter, "type": "physical", "state": "Failed"  }, "val": line.split(":")[1].strip() })'],
        },
        {
            "regex": re.compile("^Memory\s+Correctable\s+Errors\s+:"),
            "action": ['out["megacli_memory_errors"]["metrics"].append({ "labels": { "adapter": adapter, "type": "correctable"  }, "val": line.split(":")[1].strip() })'],
        },
        {
            "regex": re.compile("^Memory\s+Uncorrectable\s+Errors\s+:"),
            "action": ['out["megacli_memory_errors"]["metrics"].append({ "labels": { "adapter": adapter, "type": "uncorrectable"  }, "val": line.split(":")[1].strip() })'],
        },
    ]
    ...
    pat_pd = [
        ...
        },
        {
            ...
            "action": ['out["megacli_pd_info"]["metrics"].append({ "labels": { "adapter": adapter, "enclosure": enclosure, "slot": slot, "type": "device_id" }, "val": line.split(":")[1].strip() })'],
        },
        {
        ...
        {
            "regex":
            re.compile("^Sequence\s+Number\s*:"),
            "action":
        ...
        },
        {
            ...
            "action": ['out["megacli_pd_errors"]["metrics"].append({ "labels": { "adapter": adapter, "enclosure": enclosure, "slot": slot, "type": "media" }, "val": line.split(":")[1].strip() })'],
        },
        {
            "regex": re.compile("^Other\s+Error\s+Count\s*:"),
            "action": ['out["megacli_pd_errors"]["metrics"].append({ "labels": { "adapter": adapter, "enclosure": enclosure, "slot": slot, "type": "other" }, "val": line.split(":")[1].strip() })'],
        },
        {
        ...
        {
            "regex":
            re.compile("^Non\s+Coerced\s+Size\s*:"),
            "action": [
        ...
        {
            "regex":
            re.compile("^Connected\s+Port\s+Number\s*:"),
            "action": [
        ...
        {
            "regex":
            re.compile("^Needs\s+EKM\s+Attention\s*:"),
            "action": [
        ...
        {
            "regex":
            re.compile("^Drive\s+Temperature\s*:"),
            "action": [
        ...
        {
            "regex":
            re.compile("^Drive\s+has\s+flagged\s+a\s+S.M.A.R.T\s+alert\s*:"),
            "action":
    ...
    for line in info:
        for p in pat_info:
            if p["regex"].match(line):
                for a in p["action"]:
                    _locals = locals()
                    exec(a, globals(), _locals)
                    adapter = _locals.get("adapter", adapter)
        ...
        for p in pat_pd:
            if p["regex"].match(line):
                for a in p["action"]:
                    ...
                    exec(a, globals(), _locals)
                    adapter = _locals.get("adapter", adapter)
                    enclosure = _locals.get("enclosure", enclosure)
                    slot = _locals.get("slot", slot)
                    position = _locals.get("position", position)
                    device_id = _locals.get("device_id", device_id)
                    wwn = _locals.get("wwn", wwn)
    ...
    for k, v in out.items():
        ...
        for metric in metrics:
            print(k, metric["labels"])
            if k in GAUGES:
                gauge = GAUGES[k]
            else:
                GAUGES[k] = prometheus_client.Gauge(name=k, documentation="", labelnames=metric["labels"], registry=registry)
                gauge = GAUGES[k]

            labels = {l: "" for l in gauge._labelnames}
            labels.update(metric["labels"])
            gauge.labels(**labels).set(metric["val"])


if __name__ == "__main__":
    prometheus_client.start_http_server(9902, "0.0.0.0")
    while True:
